...
export class WebcamViewComponent implements OnInit, OnDestroy {
  ...
  private handleResults(results: any): void {
    ...
    if (results.multiHandLandmarks?.length > 0) {
      ...
      // Get normalized finger positions (0-1 relative to hand size)
      const normalized = this.normalizeLandmarks(landmarks);
      feedback = this.recognizeASL(normalized);
    }
  
    // Visual feedback
    ctx.fillStyle = feedback === "correct" ? "green" :
                   feedback === "incorrect" ? "red" : "black";
    ctx.font = "24px Arial";
    ctx.fillText(
